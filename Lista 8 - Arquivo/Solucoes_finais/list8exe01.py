...
def nomeAleatorio(nLetras): #Nomes dos candidatos
    ...
    return nome
def escreveArq1(nomeArquivo, nCandidatos, notasCotistas, inscricoesAmpla, notas): #Arquivo 1 de entrada
...
def defineNotas(notas, nCandidatos): #Definindo as notas dos canditados e colocando em um vetor
    ...
    for i in range(1,nCandidatos + 1):
        ...
        medias.append(round(notaFinal,2))
...
def pegaClassificacao(inscricao, nomeArquivo):
    ...
    while(linha != ""):
        if(inscricao in linha):
            ...
            return linha[0:4]
        linha = arquivoClassificacao.readline()
def defineClassificacao(matriz, nCandidatos, nomeArquivo, posicaoM, posicaoI, tipo, posicaoD = 2, posicaoO = 1): #Ordena em ordem decrescente
    arquivo = open(nomeArquivo, 'w')
    if(tipo == 1):
    ...
    while(matriz[posicaoM].count(-1) != nCandidatos + 1):
        ...
        if(tipo == 1):
            ...
            arquivo.write(nomeCandidato + "," + "0"*(4 - len(classInicial)) + classInicial + "," + "0"*(4 - len(classFinal)) + classFinal + "\n")
